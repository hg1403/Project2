...
def view_cart(request):
    if request.session.has_key('userlogin'):
        ...
        cart_items = CartItems.objects.filter(user=user_id, buy_now=False).select_related('products_id').order_by('id')
        categories = Category.objects.all()
        subcategories = SubCategory.objects.all()
        cart_sub_total = sum(item.sub_total for item in cart_items)
...
def wishlist(request):
    if request.session.has_key('userlogin'):
        id = request.user.id
        wishlist_items = Wishlist.objects.filter(user=id).order_by('id')
        categories = Category.objects.all()
        subcategories = SubCategory.objects.all()
        products = Products.objects.all()
